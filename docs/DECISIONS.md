...
## D-039 — Bounded physical infrastructure hazards and structural route restrictions
**Decision:** Sprint 14 formalises infrastructure hazard conditions across three concrete types: (1) Damaged / Jammed Points (turnouts locked in alignment until repaired by crew via `TASK_REPAIR_POINT`); (2) Restricted Bridges and Track Sections (structural mass limits such as 240t evaluated via `RouteRequirementEvaluator`); (3) Damaged Track Sections (unusable segments repaired by crew via `TASK_REPAIR_TRACK` consuming 2 parts and labor). Worldgen production generator deterministically places hazards and structural limits using `STREAM_GAMEPLAY_PROBLEM` subseed. Solvability is guaranteed by construction: every generated sector provides either reachable repair resources (POIs / train baseline) or an open alternate route. Sector departure UI dynamically displays blockers, unboarded crew warnings, and actionable hints.
**Reason:** Railway hazards should create real operational problems solved through physical train shunting, route choices, and crew labor, avoiding scripted quest triggers while keeping railway simulation and sector lifecycle authorities cleanly decoupled.

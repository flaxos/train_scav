...
func _init() -> void:
	...
	var tutorial_text := "\n".join(lines)
	_expect(tutorial_text.contains("Train Scav - Sprint 12 UAT"), "scene has an in-game UAT guide")
	_expect(tutorial_text.contains("Sprint 11 Procgen Check"), "UAT guide includes the current sprint load check")
	_expect(tutorial_text.contains("mobility summary and route requirements"), "UAT guide explains the current goal")
	...
	_expect(tutorial_text.contains("Departure requires all crew aboard"), "UAT guide explains crew return and diesel before departure")
